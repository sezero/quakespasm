...
Vendored in from ironwail's common.c (Quake/common.c) -- QuakeSpasm
doesn't have this helper, and this is the only json.c call site that
needs it (decoding \uXXXX escapes). Writes a single Unicode code point
using UTF-8; returns the number of bytes written (up to 4), or 0 on
error (overflow or invalid code point).
==================
*/
static size_t UTF8_WriteCodePoint (char *dst, size_t maxbytes, uint32_t codepoint)
{
	if (!maxbytes)
		return 0;
